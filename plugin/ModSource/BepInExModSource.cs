	...
	public class BepInExModSource : IModSource {
		public IEnumerable<LocalMod> GetMods() {
			...
			foreach (PluginInfo info in Chainloader.PluginInfos.Values) {
				...
				mods.Add(new LocalMod(FromBepInPlugin(info.Metadata, Path.GetDirectoryName(dllPath)), Path.GetFileNameWithoutExtension(dllPath), folderName));
			}
			return mods;
		}
		...
		private ModInfo FromBepInPlugin(BepInPlugin plugin, string localPath) {
			return new ModInfo(plugin.GUID, plugin.Name, plugin.Version, null, null, null, localPath);
		}
	}
}